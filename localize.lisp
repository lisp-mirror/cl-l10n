...
(defun add-resource (locale name args &rest body)
...
(defun lookup-resource-with-fallback (name args &rest rest &key &allow-other-keys)
  (loop for locale in (or *fallback-locales* (list *locale*)) do
        (let ((result (apply 'lookup-resource locale name args rest)))
          (when result
            (return result)))))
